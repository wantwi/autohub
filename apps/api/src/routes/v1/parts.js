...
async function createPartForDealer({ pool, dealerId, body, createdByUserId, createdByRole }) {
  ...
  const sql = range
    ...
    : `INSERT INTO parts (
         dealer_id, name, description, category, condition, price, quantity,
         compatible_makes, compatible_models, images, part_number, created_by_user_id, created_by_role, is_negotiable
...
adminPartsRouter.patch('/dealers/:dealerId/listing-limit', requireAuth, requireRole('admin'), async (req, res, next) => {
  try {
    const { limit } = z.object({ limit: z.coerce.number().int().min(1) }).parse(req.body);
    const pool = getPool();
    const { rows } = await pool.query(
      'UPDATE dealers SET listing_limit = $1 WHERE id = $2 RETURNING id, listing_limit',
      [limit, req.params.dealerId],
    );
